...
#include<string.h>
#include<stdlib.h>
#define MAX 1150 //save node max
#define Queue_MAX 1000000 //給queue用的
#define max 10000
int Nodes,Links,Req; 
int NodeID,QM;
...
int BFS(int Node,int ReqSrc,int ReqDst){ // (節點數量,開始的節點,結束的節點)
    ...
    struct QItem* queue_save = (struct QItem*)malloc(MAX*sizeof(struct QItem));
...
void delete_EN(int *arr,int End,int ID){ //等帶判斷函式的回傳值 決定要不要扣Memories 跟 Link
    ...
    if (result == 1){
        ...
        reject_path[reject_counter][0] = arr[0];
        reject_path[reject_counter][1] = arr[End];
        reject_ID[reject_counter] = ID; //idnex 0 save req ID
        reject_counter++;
    }
}
void inital_list(){
...
int main(){
    ...
    for (int i = 0; i < reject_counter; i++){
        test = BFS(Nodes,reject_path[i][0],reject_path[i][1]);

        delete_EN(first_answer,test,reject_ID[i]);
    }
