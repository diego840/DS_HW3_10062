...
int BFS(int Node,int ReqSrc,int ReqDst){ // (節點數量,開始的節點,結束的節點)
    ...
    Counter = queue_save[ReqDst].Location; //存從reqSrc到reqDst的distance
    //printf("%d \n",Counter);
    tmp = ReqDst; 
    for (int k = 0; k <= Counter; k++){ //從reqDst一路找回去到reqSrc所經過的節點
        first_answer[k] = tmp; //路徑暫時存放的list
        tmp = queue_save[tmp].parent_value;
    }
    return Counter; //回傳reqDst的distance
}
...
int main(){
    ...
    for (int i = 0; i < Req; i++){
        test = BFS(Nodes,Req_list[i][1],Req_list[i][2]);//尋找每條req的路徑 回傳值是reqDst的distance
        end_number[i] = test; 
        //printf("test:%d \n",test);
        for (int j = 0,k= test; j <= test; k--,j++){ //將存路徑的list反轉
            shortest_path[i][j] =  first_answer[k]; 
        }    
    }

    for (int i = 0; i < Req; i++){ //檢查路徑是否可以走
        delete_EN(shortest_path[i],end_number[i],Req_list[i][0]);
    }
    
    printf("%d\n",answer_counter); //輸出可以走的路徑
